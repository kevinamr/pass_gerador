...
app.post('/signup', async (req, res) => {
    try {
        ...
        const [usuariosExistentes] = await db.execute(
            'SELECT id FROM pass_gerador.usuarios WHERE email = ?',
            [emailFormatado]
        );
        ...
        await db.execute(
            'INSERT INTO pass_gerador.usuarios (nome, email, senha) VALUES (?, ?, ?)',
...
app.post('/signin', async (req, res) => {
    try {
        ...
        const [usuarios] = await db.execute(
            'SELECT id, nome, email, senha FROM pass_gerador.usuarios WHERE email = ?',
            [emailFormatado]
        );
...
async function ensureSchema() {
    try {
        await db.execute(`
            CREATE TABLE IF NOT EXISTS pass_gerador.usuarios (
        ...
        await db.execute(`
            CREATE TABLE IF NOT EXISTS pass_gerador.historico (
                id INT AUTO_INCREMENT PRIMARY KEY,
                usuario_id INT NOT NULL,
                nome_aplicativo VARCHAR(255) NOT NULL,
                senha VARCHAR(255) NOT NULL,
                criado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                CONSTRAINT fk_historico_usuario FOREIGN KEY (usuario_id) REFERENCES pass_gerador.usuarios(id) ON DELETE CASCADE
            ) ENGINE=InnoDB;
        `);
...
app.post('/historico', requireAuth, async (req, res) => {
    try {
        ...
        await db.execute(
            'INSERT INTO pass_gerador.historico (usuario_id, nome_aplicativo, senha, criado_em) VALUES (?, ?, ?, NOW())',
...
app.get('/historico', requireAuth, async (req, res) => {
    try {
        ...
        const query = 'SELECT id, nome_aplicativo AS nomeAplicativo, senha, criado_em FROM pass_gerador.historico WHERE usuario_id = ? ORDER BY criado_em DESC';
...
app.delete('/historico/:id', requireAuth, async (req, res) => {
    try {
        ...
        const [result] = await db.execute('DELETE FROM pass_gerador.historico WHERE id = ? AND usuario_id = ?', [id, usuarioId]);
